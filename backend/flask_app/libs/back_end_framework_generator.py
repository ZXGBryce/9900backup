...
class FrameworkGenerator:
    def __init__(self, simplified = True):
        ...
        if simplified:
            ...
            self.frameworks_dict = {"TCFD":{"Transition Risk": 
                        ...
                        "Chronic":
                        {"Rising mean temperatures":{}}}}}
        self.add_weights_to_framework(simplified)
    ...
    def add_weights_to_framework(self, simplified = True):
        ...
        if simplified:
            for framework, metrics_dict in self.frameworks_dict.items():
                for metric, indicator_dict in metrics_dict.items():
                    number_of_indicators_in_metric = len(indicator_dict)
                    for indicator in indicator_dict.keys():
                        indicator_dict[indicator] = int(
